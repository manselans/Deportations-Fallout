"""
Collect data on partnered/married, foreign-born females.
"""
...
def run():
    ...
    ffb = (
        fetch(
            paths.data.dst_raw / "bef12_2010.dta",
            columns=[
                "pnr",
                "alder",
                "antboernf",
    ...
    ffb = (
        ...
        .assign(
            convicted=lambda d: d["pnr"].isin(
                fetch(  # penal crime convictions, 2010
                    paths.data.dst_raw / "kraf2010.dta",
                    columns=["pnr"],
            ...
            charged=lambda d: d["pnr"].isin(
                fetch(  # penal crime charges, 2010
                    paths.data.dst_raw / "krsi2010.dta",
                    columns=["pnr"],
        ...
        .merge(
            fetch(
                paths.data.dst_raw / "bfl2010.dta",
        ...
        .merge(
            fetch(
                paths.data.dst_raw / "ilme12_2010.dta",
        ...
        .merge(
            fetch(
                paths.data.dst_raw / "ind2010.dta",
    ...
    ffb.loc[ffb["statsb"].isin(eu_eea) & ~ffb["statsb"].isin(kingdom), "grounds"] = 1

    # From OPHG
    mis = ffb["grounds"].isna()
    ffb.loc[mis & ffb["kategori"].eq(4), "grounds"] = 1  # EU/EEA
    ffb.loc[mis & ffb["kategori"].eq(1), "grounds"] = 2  # Asylum
    ffb.loc[mis & ffb["kategori"].isin([3, 6]), "grounds"] = 3  # Study/work
    ffb.loc[mis & ffb["kategori"].eq(5), "grounds"] = 4  # Family Reunified
    ffb.loc[mis & ffb["kategori"].eq(2), "grounds"] = 5  # Unspecified

    ffb.drop(columns=["kategori"], inplace=True)

    ffb.to_parquet(paths.temp / "female_foreignborn.parquet")
