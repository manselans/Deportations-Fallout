"""
Create Figure A1: Main Results for Unconditional Prison Sentences Only.
"""
...
def figure_a1():
    ...
    imprisoned = convicts_df.loc[
        lambda d: d["D"].eq(1) | d["prison_sentence"].gt(0), ["pnr", "conviction_date"]
    ].rename(columns={"pnr": "convict"})

    imprisoned["conviction_date"] = imprisoned["conviction_date"].dt.to_period("M")
    imprisoned = imprisoned.drop_duplicates()

    df = panel_df.merge(
        imprisoned, on=["convict", "conviction_date"], how="inner", validate="m:1"
    )[["pnr", "D", "event_time", "partic", "fulltime", "wages", "transfers"]]
