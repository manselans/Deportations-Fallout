...
def table_by_two(
    ...
    *,
    ...
    # Discretion
    if not ("mean" in stats and "count" in stats):
        warnings.warn(
            "`mean` and `count` must both be included in stat for discretion to be ensured!"
        )
    else:
        for g in levels:
            _n = (f"{g}", "count")
            mean = (f"{g}", "mean")
            too_few = tbl[mean].between(0, 1) & (tbl[mean] * tbl[_n]).between(1, 2)
            too_many = tbl[mean].between(0, 1) & ((1 - tbl[mean]) * tbl[_n]).between(
                1, 2
            )
            tbl.loc[too_few | too_many] = np.nan

    #tbl = tbl.apply(_format_col)
...
def _format_col(s):
    mask = s.between(0, 1)
    out = s.where(mask, s.round(0)).where(~mask, s.round(2))
    return out.astype("Int64") if not mask.any() else out
